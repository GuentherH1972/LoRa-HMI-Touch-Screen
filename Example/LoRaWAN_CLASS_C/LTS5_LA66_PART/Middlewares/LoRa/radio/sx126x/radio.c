    ...
    if (IrqFired == true)
    {
        ...
        if ((irqRegs & IRQ_RX_DONE) == IRQ_RX_DONE)
        {
            ...
            if ((RadioEvents != NULL) && (RadioEvents->RxDone != NULL) && ((irqRegs & IRQ_CRC_ERROR) != IRQ_CRC_ERROR))
            {
                ...
                record_rssi(RadioPktStatus.Params.LoRa.RssiPkt + RadioPktStatus.Params.LoRa.SnrPkt);
                LOG_PRINTF(LL_DEBUG, "rxDone\n\r");
#if !defined(LA66_HARDWARE_TEST)
                LOG_PRINTF(LL_DEBUG, "Rssi= %d\r", RadioPktStatus.Params.LoRa.RssiPkt + RadioPktStatus.Params.LoRa.SnrPkt);
#else
                LOG_PRINTF(LL_DEBUG, "Rssi= %d dbm\r", RadioPktStatus.Params.LoRa.RssiPkt + RadioPktStatus.Params.LoRa.SnrPkt);
#endif
            }
        }
